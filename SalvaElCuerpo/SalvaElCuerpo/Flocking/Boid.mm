...
@synthesize bottomEdge = _bottomEdge;
@synthesize topEdge = _topEdge;
...
-(void) handleBorder
{
	...
	{
		if(_internalPosition.x < _leftEdge) [self setPos:ccp(_rightEdge, self.position.y)];
		else if (_internalPosition.y < _bottomEdge) [self setPos:ccp(_internalPosition.x, _topEdge)];
		else if (_internalPosition.x > _rightEdge) [self setPos:ccp(_leftEdge, _internalPosition.y)];
		else if (_internalPosition.y > _topEdge) [self setPos:ccp(_internalPosition.x, _bottomEdge)];
	} 
	else if (_edgeBehavior == EDGE_BOUNCE)
	{	
		if(_internalPosition.x < _leftEdge) {_internalPosition.x = _leftEdge; _velocity.x *= -1.0f;}
		else if (_internalPosition.x > _rightEdge) {_internalPosition.x = _rightEdge; _velocity.x *= -1.0f;}
		if (_internalPosition.y < _bottomEdge) { _internalPosition.y = _bottomEdge; _velocity.y *= -1.0f;}
		else if (_internalPosition.y > _topEdge) { _internalPosition.y = _topEdge; _velocity.y *= -1.0f;}
	}
}

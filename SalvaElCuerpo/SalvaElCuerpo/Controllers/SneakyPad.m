...
@interface SneakyPad(hidden)
- (void)updateVelocity:(CGPoint)point;
- (void)resetJoystick;
@end


@implementation SneakyPad
...
-(void)updateVelocity:(CGPoint)point {
	...
	//global._joystick_velocity = velocity;
	//global._joystick_address = tag;
...
-(id)initWithRect:(CGRect)rect{
	...
	if(self){
		...
		background = [CCSprite spriteWithFile:@"128_white.png"];
		[background setPosition:center];
		[self addChild:background z:0];
		
		thumb = [CCSprite spriteWithFile:@"64_white.png"];
